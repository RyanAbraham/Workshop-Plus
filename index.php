 ...
 */
require_once(dirname(dirname(dirname(__FILE__))).'/../workshop/config.php');
...
if (! $workshops = get_all_instances_in_course('workshopplus', $course)) {
    echo $OUTPUT->heading(get_string('modulenameplural', 'workshopplus'));
    notice(get_string('noworkshops', 'workshopplus'), new moodle_url('/course/view.php', array('id' => $course->id)));
    echo $OUTPUT->footer();
    die();
}
...
foreach ($workshops as $workshop) {
    if (empty($workshop->visible)) {
        $link = html_writer::link(new moodle_url('/mod/workshopplus/view.php', array('id' => $workshop->coursemodule)),
