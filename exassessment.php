...
$workshop   = $DB->get_record('workshopplus', array('id' => $example->workshopid), '*', MUST_EXIST);
$course     = $DB->get_record('course', array('id' => $workshop->course), '*', MUST_EXIST);
$cm         = get_coursemodule_from_instance('workshopplus', $workshop->id, $course->id, false, MUST_EXIST);
...
if ($assessmenteditable and $workshop->overallfeedbackmode) {
    $currentdata = file_prepare_standard_editor($currentdata, 'feedbackauthor', $workshop->overall_feedback_content_options(),
        $workshop->context, 'mod_workshopplus', 'overallfeedback_content', $assessment->id);
    if ($workshop->overallfeedbackfiles) {
        $currentdata = file_prepare_standard_filemanager($currentdata, 'feedbackauthorattachment',
            $workshop->overall_feedback_attachment_options(), $workshop->context, 'mod_workshopplus', 'overallfeedback_attachment',
            $assessment->id);
    }
}
...
} else if ($assessmenteditable and ($data = $mform->get_data())) {
...
// output starts here
$output = $PAGE->get_renderer('mod_workshopplus');      // workshop renderer
echo $output->header();
