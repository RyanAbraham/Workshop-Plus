...
class workshopplus_submission_form extends moodleform {
    ...
    function validation($data, $files) {
        ...
        if (empty($data['id']) and empty($data['example'])) {
            ...
            $sql = "SELECT COUNT(s.id)
                      ...
                      JOIN {workshopplus} w ON (s.workshopid = w.id)
            ...
            if ($DB->count_records_sql($sql, array($data['cmid'], $USER->id))) {
                $errors['title'] = get_string('err_multiplesubmissions', 'mod_workshopplus');
            }
        }

        if (isset($data['attachment_filemanager']) and isset($this->_customdata['workshopplus']->submissionfiletypes)) {
            $whitelist = workshop::normalize_file_extensions($this->_customdata['workshopplus']->submissionfiletypes);
            if ($whitelist) {
                ...
                if ($draftfiles) {
                    ...
                    if ($wrongfiles) {
                        ...
                        $errors['attachment_filemanager'] = get_string('err_wrongfileextension', 'mod_workshopplus', $a);
                    }
                }
            }
        }
