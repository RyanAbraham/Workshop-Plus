...
if ($id) {
    ...
    $workshoprecord = $DB->get_record('workshopplus', array('id' => $cm->instance), '*', MUST_EXIST);
} else {
    $workshoprecord = $DB->get_record('workshopplus', array('id' => $w), '*', MUST_EXIST);
    $course         = $DB->get_record('course', array('id' => $workshoprecord->course), '*', MUST_EXIST);
    $cm             = get_coursemodule_from_instance('workshopplus', $workshoprecord->id, $course->id, false, MUST_EXIST);
}
...
$PAGE->set_url($workshop->view_url2());
$event = \mod_workshop\event\course_module_viewed::create($eventdata);
$event->add_record_snapshot('course', $course);
$event->add_record_snapshot('workshopplus', $workshoprecord);
...
if ($workshop->phase == workshop::PHASE_SUBMISSION and $workshop->phaseswitchassessment
    ...
    $DB->set_field('workshopplus', 'phaseswitchassessment', 0, array('id' => $workshop->id));
...
echo $output->header();
echo $output->heading_with_help(format_string($workshop->name), 'userplan', 'workshopplus');
echo $output->render($userplan);

switch ($workshop->phase) {
    case workshop::PHASE_SETUP:
        if (trim($workshop->intro)) {
            print_collapsible_region_start('', 'workshop-viewlet-intro', get_string('introduction', 'workshopplus'));
            echo $output->box(format_module_intro('workshopplus', $workshop, $workshop->cm->id), 'generalbox');
            print_collapsible_region_end();
        }
        if ($workshop->useexamples and has_capability('mod/workshop:manageexamples', $PAGE->context)) {
            print_collapsible_region_start('', 'workshop-viewlet-allexamples', get_string('examplesubmissions', 'workshopplus'));
    ...
    case workshop::PHASE_SUBMISSION:
        ...
        if ($workshop->assessing_examples_allowed()
            ...
            print_collapsible_region_start('', 'workshop-viewlet-examples', get_string('exampleassessments', 'workshopplus'), false, $examplesdone);
        ...
        if (has_capability('mod/workshop:submit', $PAGE->context) and (!$examplesmust or $examplesdone)) {
            print_collapsible_region_start('', 'workshop-viewlet-ownsubmission', get_string('yoursubmission', 'workshopplus'));
        ...
        if (has_capability('mod/workshop:viewallsubmissions', $PAGE->context)) {
            ...
            print_collapsible_region_start('', 'workshop-viewlet-allsubmissions', get_string('submissionsreport', 'workshopplus'));
            ...
            if ($data) {
                ...
                echo html_writer::tag('div', get_string('submittednotsubmitted', 'workshopplus', $a));
                ...
                echo html_writer::tag('div', get_string('nothingfound', 'workshopplus'), array('class' => 'nothingfound'));
            }
            print_collapsible_region_end();
        }
        break;
    case workshop::PHASE_ASSESSMENT:
        ...
        if (has_capability('mod/workshop:submit', $PAGE->context)) {
            if ($ownsubmission = $workshop->get_submission_by_author($USER->id)) {
                print_collapsible_region_start('', 'workshop-viewlet-ownsubmission', get_string('yoursubmission', 'workshopplus'), false, true);
                echo $output->box_start('generalbox ownsubmission');
        ...
        if (has_capability('mod/workshop:viewallassessments', $PAGE->context)) {
            ...
            if ($data) {
                ...
                print_collapsible_region_start('', 'workshop-viewlet-gradereport', get_string('gradesreport', 'workshopplus'));
        ...
        if (trim($workshop->instructreviewers)) {
            $instructions = file_rewrite_pluginfile_urls($workshop->instructreviewers, 'pluginfile.php', $PAGE->context->id,
                'mod_workshopplus', 'instructreviewers', 0, workshop::instruction_editors_options($PAGE->context));
            print_collapsible_region_start('', 'workshop-viewlet-instructreviewers', get_string('instructreviewers', 'workshopplus'));
        ...
        if (!$examplesdone and $examplesmust and ($ownsubmissionexists === false)) {
            print_collapsible_region_start('', 'workshop-viewlet-examplesfail', get_string('exampleassessments', 'workshopplus'));
        ...
        if (!$examplesmust or $examplesdone) {
            print_collapsible_region_start('', 'workshop-viewlet-assignedassessments', get_string('assignedassessments', 'workshopplus'));
    ...
    case workshop::PHASE_EVALUATION:
        ...
        if (has_capability('mod/workshop:overridegrades', $workshop->context)) {
            print_collapsible_region_start('', 'workshop-viewlet-cleargrades', get_string('toolbox', 'workshopplus'), false, true);
    ...
    case workshop::PHASE_CLOSED:
        if (trim($workshop->conclusion)) {
            $conclusion = file_rewrite_pluginfile_urls($workshop->conclusion, 'pluginfile.php', $workshop->context->id,
                'mod_workshopplus', 'conclusion', 0, workshop::instruction_editors_options($workshop->context));
            print_collapsible_region_start('', 'workshop-viewlet-conclusion', get_string('conclusion', 'workshopplus'));
        ...
        if (!empty($finalgrades)) {
            print_collapsible_region_start('', 'workshop-viewlet-yourgrades', get_string('yourgrades', 'workshopplus'));
        ...
        if (has_capability('mod/workshop:viewpublishedsubmissions', $workshop->context)) {
            ...
            if ($submissions = $workshop->get_published_submissions()) {
                print_collapsible_region_start('', 'workshop-viewlet-publicsubmissions', get_string('publishedsubmissions', 'workshopplus'));
