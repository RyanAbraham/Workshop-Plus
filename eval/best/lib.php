...
class workshopplus_best_evaluation extends workshop_evaluation {
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        } else if ($this->settings->comparison != $settings->comparison) {
        ...
        $this->update_grading_grades_process($settings, $restrict, $rsex, $diminfo, false);
        $rsex->close();
        ...
        if ($this->workshop->phase != workshop::PHASE_SUBMISSION) {
            ...
            // process those assessments
            $this->update_grading_grades_process($settings, $restrict, $rs, $diminfo, true);
            $rs->close();
        }
    }
    /**
    ...
    public function update_grading_grades_process(stdclass $settings, $restrict=null, mysqli_native_moodle_recordset $rs, array $diminfo, $isexample) {
        ...
        foreach ($rs as $current) {
            ...
            if ($current->submissionid == $previous->submissionid) {
                ...
                $this->process_assessments($batch, $diminfo, $settings, $isexample);
     ...
     * @return void
     */
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings, $isexample) {
        global $DB;
        ...
        // calculate the grading harshness
        $gradinghs = array();
        foreach ($assessments as $a) {
            if ($a->weight == 1) {
                foreach ($assessments as $asid => $assessment) {
                    ...
                    echo "<font color='dark pink'>grading harshness: ";
                    print_r($h);
                    echo "</font><br>";

                    if (!is_null($h) and (!isset($gradinghs[$asid]) or $h < $gradinghs[$asid])) {
                        $gradinghs[$asid] = $h;
                    }
                }
            }
        ...
        // we do not use set_field() here because we want to pass $bulk param
        foreach ($gradinghs as $assessmentid => $gradingh) {
            echo "<font color='dark red'>assessments: ";
            print_r($assessments[$assessmentid]);
            echo "</font><br>";
            if (grade_floats_different($gradingh, $assessments[$assessmentid]->gradingharshness)) {
                // the value has changed
                $record = new stdclass();
                $record->id = $assessmentid;
    ...
    protected function assessments_distance(stdclass $assessment, stdclass $referential, array $diminfo, stdclass $settings) {
        ...
        if ($n > 0) {
            ...
            echo "<font color='burgundy'>Distance of</font> <font color='orange'>$agrade</font><font color='burgundy'> from
               </font><font color='orange'>$rgrade</font><font color='burgundy'>:</font> <font color='orange'>$temp</font><br><br>";
     ...
     * by -1 or in other words, used to calculate the grading harshness later on
     * @param stdClass $assessment the assessment being measured
    ...
    protected function calc_harshness(stdclass $assessment, stdclass $reference, array $diminfo) {
        ...
        // return -1 or 1 to be multiplied later in the calculation of the grading harshness
