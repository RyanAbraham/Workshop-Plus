 ...
 * Allocates the submissions randomly
 */
class workshopplus_random_allocator implements workshop_allocator {
     ...
     *
     * @param workshopplus_random_allocator_setting $setting
     * @param workshop_allocation_result allocation result logger
     */
    public function execute(workshopplus_random_allocator_setting $settings, workshop_allocation_result $result) {
        ...
        $gradingharshnesss = $this->workshop->get_all_harshness_scores();
        ...
        if ($settings->numofreviews) {
            ...
            $randomallocations  = $this->random_allocation($authors, $reviewers, $curassessments,
                                                            $gradingharshnesss, $result, $options);
     ...
     * @param array    $gradingharshnessesraw    structure of grading harshnesses
     ...
     */
    protected function random_allocation($authors, $reviewers, $assessments, array $gradinggradesraw, array $gradingharshnesssraw, $result, array $options) {
        ...
        $gradingharshnesssid = array();

        // Extract the grading harshnesss and attach them to the respective students
        // The grading harshnesss are simply summed up
        foreach ($gradingharshnesssraw as $gradingharshnesss) {
            foreach ($reviewers[0] as $reviewer) {
                if ($reviewer->id == $gradingharshnesss->reviewerid) {
                    // If a grade was already added to the array for this id
                    if (isset($gradingharshnesssid[$reviewer->id])) {
                        $gradingharshnesssid[$reviewer->id] += $gradingharshnesss->gradingharshness; // Add the grading grade
                    } else { // For the first grade to be added to the array for each id, += can't be used
                        $value = $gradingharshnesss->gradingharshness;
                        echo "val: $value<br>";
                        $gradingharshnesssid[$reviewer->id] = $gradingharshnesss->gradingharshness;
                    }
                }
            }
        }
        // Sort the grading harshnesss from lowest to highest
        asort($gradingharshnesssid);
        print_r($gradingharshnesssid);

        // Determine the pattern for allocating a range of grading grades
        // This pattern is used by get_element_with_lowest_workload
        $i = 0;
        // If it's odd, a 'MID' has to be added to the pattern because
        // there must always be the same number of HIGH and LOW entries
        if ($numofreviews % 2 == 1) {
            $pattern[0] = 'MID';
            $i++;
        }
        for (; $i < $numofreviews; $i++) {
            // Alternate between adding 'HIGH' and 'LOW' to the array
            $pattern[$i] = ($i % 2 == 0 ? 'HIGH' : 'LOW');
        }
        echo "<BR><BR>PATTERN: <BR>";
        print_r($pattern);
        ...
        } else if (workshopplus_random_allocator_setting::NUMPER_REVIEWER == $numper) {
        ...
        foreach ($circlegroups as $circlegroupid) {
            ...
            for ($requiredreviews = 1; $requiredreviews <= $numofreviews; $requiredreviews++) {
                echo "<h1><br>REQUIRED: $requiredreviews</h1>";
        ...
        $fairness = calculate_fairness($returned);
        return $returned;
    }
     ...
     */
    protected function get_element_with_lowest_workload($workload, $gradinggrades, $pattern) {
        $precision = 10;
        ...
        if ($pattern == 'HIGH') {
            // Flip it so now it's sorted from highest to lowest
            $gradinggrades = array_reverse($gradinggrades, true);
        }
        echo "<BR>MIN_KEYS: ";
        print_r($minkeys);
        echo "<BR>GRADING_KEYS: ";
        print_r($gradinggrades);
        // Find the first id in $gradinggrades with lowest workload
        ...
        // If something went wrong and hasn't returned yet, just return a random element
 ...
 */
class workshopplus_random_allocator_setting {
     ...
     * @return workshopplus_random_allocator_setting
     */
