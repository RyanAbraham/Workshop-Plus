 ...
 */
function workshopplus_add_instance(stdclass $workshop) {
    global $CFG, $DB;
    ...
    $workshop->id = $DB->insert_record('workshopplus', $workshop);
    ...
    if ($draftitemid = $workshop->instructauthorseditor['itemid']) {
        $workshop->instructauthors = file_save_draft_area_files($draftitemid, $context->id, 'mod_workshopplus', 'instructauthors',
    ...
    if ($draftitemid = $workshop->instructreviewerseditor['itemid']) {
        $workshop->instructreviewers = file_save_draft_area_files($draftitemid, $context->id, 'mod_workshopplus', 'instructreviewers',
    ...
    if ($draftitemid = $workshop->conclusioneditor['itemid']) {
        $workshop->conclusion = file_save_draft_area_files($draftitemid, $context->id, 'mod_workshopplus', 'conclusion',
    ...
    $DB->update_record('workshopplus', $workshop);
 ...
 * @return bool success
 */
function workshopplus_update_instance(stdclass $workshop) {
    global $CFG, $DB;
    require_once(dirname(__FILE__) . '/locallib.php');
...
function workshopplus_delete_instance($id) {
    ...
    $submissions = $DB->get_records('workshopplus_submissions', array('workshopid' => $workshop->id), '', 'id');
 ...
 * @return array
 */
function workshopplus_get_view_actions() {
 ...
 * @return array
 */
function workshopplus_get_post_actions() {
 ...
 */
function workshopplus_user_outline($course, $user, $mod, $workshop) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
 ...
 * @return string HTML
 */
function workshopplus_user_complete($course, $user, $mod, $workshop) {
    global $CFG, $DB, $OUTPUT;
    require_once(dirname(__FILE__).'/locallib.php');
    require_once($CFG->libdir.'/gradelib.php');

    $workshop   = new workshopplus($workshop, $mod, $course);
    $grades     = grade_get_grades($course->id, 'mod', 'workshop', $workshop->id, $user->id);
...
function workshopplus_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    $sql = "SELECT s.id AS submissionid, s.title AS submissiontitle, s.timemodified AS submissionmodified,
        ...
        INNER JOIN {workshopplus_submissions} s ON s.workshopid = w.id
 ...
 * @return bool
 */
function workshopplus_scale_used($workshopid, $scaleid) {
 ...
 * @return void
 */
function workshopplus_grade_item_update(stdclass $workshop, $submissiongrades=null, $assessmentgrades=null) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');
 ...
 * @return void
 */
function workshopplus_update_grades(stdclass $workshop, $userid=0) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
 ...
 * @param stdClass $workshop An object from the form in mod_form.php
 */
function workshopplus_grade_item_category_update($workshop) {
...
function workshopplus_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    ...
    if ($filearea === 'instructauthors') {
        ...
        if (!$submission = $DB->get_record('workshopplus_submissions', array('id' => $itemid, 'workshopid' => $workshop->id))) {
            return false;
        }
        ...
        if (!$submission = $DB->get_record('workshopplus_submissions', array('id' => $assessment->submissionid, 'workshopid' => $workshop->id))) {
            return false;
        }
 ...
 */
function workshopplus_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $workshopnode=null) {
    global $PAGE;
...
function workshopplus_page_type_list($pagetype, $parentcontext, $currentcontext) {
    $modulepagetype = array('mod-workshop-*' => get_string('page-mod-workshop-x', 'workshop'));
 ...
 */
function workshopplus_calendar_update(stdClass $workshop, $cmid) {
    global $DB;
    ...
    if ($workshop->submissionstart) {
        $event = clone($base);
        $event->name = get_string('submissionstartevent', 'mod_workshopplus', $workshop->name);
    ...
    if ($workshop->submissionend) {
        $event = clone($base);
        $event->name = get_string('submissionendevent', 'mod_workshopplus', $workshop->name);
    ...
    if ($workshop->assessmentstart) {
        $event = clone($base);
        $event->name = get_string('assessmentstartevent', 'mod_workshopplus', $workshop->name);
    ...
    if ($workshop->assessmentend) {
        $event = clone($base);
        $event->name = get_string('assessmentendevent', 'mod_workshopplus', $workshop->name);
 ...
 */
function workshopplus_reset_course_form_definition($mform) {

    $mform->addElement('header', 'workshopheader', get_string('modulenameplural', 'mod_workshopplus'));

    $mform->addElement('advcheckbox', 'reset_workshopplus_submissions', get_string('resetsubmissions', 'mod_workshopplus'));
    $mform->addHelpButton('reset_workshopplus_submissions', 'resetsubmissions', 'mod_workshopplus');

    $mform->addElement('advcheckbox', 'reset_workshop_assessments', get_string('resetassessments', 'mod_workshopplus'));
    $mform->addHelpButton('reset_workshop_assessments', 'resetassessments', 'mod_workshopplus');
    $mform->disabledIf('reset_workshop_assessments', 'reset_workshopplus_submissions', 'checked');

    $mform->addElement('advcheckbox', 'reset_workshop_phase', get_string('resetphase', 'mod_workshopplus'));
    $mform->addHelpButton('reset_workshop_phase', 'resetphase', 'mod_workshopplus');
}
 ...
 */
function workshopplus_reset_course_form_defaults(stdClass $course) {

    $defaults = array(
        'reset_workshopplus_submissions'    => 1,
 ...
 */
function workshopplus_reset_userdata(stdClass $data) {
    global $CFG, $DB;

    if (empty($data->reset_workshopplus_submissions)
    ...
    foreach ($workshoprecords as $workshoprecord) {
        ...
        $workshop = new workshopplus($workshoprecord, $cm, $course);
